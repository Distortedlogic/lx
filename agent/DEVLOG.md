...
- **`Agent` is a keyword**. Lexed like `Protocol`/`Trait` (uppercase → TypeName special-case). Produces `Stmt::AgentDecl` and `Value::Class { kind: Agent }`. Methods stored in `IndexMap<String, Value>`. Reserved fields: `uses`, `init`, `on`. Trait conformance validated at definition time (missing method = hard runtime error, not catchable with `??`). Session 64: `Value::Agent` eliminated — Agent is now `Value::Class` with `ClassKind::Agent`.
- **`Protocol` is a `Value::Trait` with non-empty fields**. Session 64: `Value::Protocol` eliminated — Protocol declarations produce `Value::Trait` where `fields` is non-empty. Callable as constructor (Protocol semantics). Display: `<Protocol X>` for Traits-with-fields, `<Trait X>` for behavioral Traits.
