...
impl<R: serde::Serialize + Send + Sync> BinaryInferenceClient<R> {
  pub fn new(base_url: &str, timeout_secs: u64) -> Result<Self> {
    ...
    let request_client = reqwest::Client::builder().timeout(Duration::from_secs(timeout_secs)).build()?;
    let health_client = reqwest::Client::builder().timeout(Duration::from_secs(HEALTH_TIMEOUT_SECS)).build()?;
...
impl<R: serde::Serialize + Send + Sync> InferenceClient for BinaryInferenceClient<R> {
  ...
  async fn infer(&self, req: &Self::Request) -> Result<Self::Response> {
    ...
    if !resp.status().is_success() {
      bail!("inference failed with status {}: {}", resp.status(), resp.text().await.unwrap_or_default());
    }
