...
pub fn walk_field_access<V: AstVisitor + ?Sized>(v: &mut V, expr: &SExpr, field: &FieldKind, _span: Span) {
...
pub fn walk_block<V: AstVisitor + ?Sized>(v: &mut V, stmts: &[SStmt], span: Span) {
  for s in stmts {
...
pub fn walk_func<V: AstVisitor + ?Sized>(v: &mut V, params: &[Param], ret_type: Option<&SType>, body: &SExpr, span: Span) {
  for p in params {
...
pub fn walk_match<V: AstVisitor + ?Sized>(v: &mut V, scrutinee: &SExpr, arms: &[MatchArm], span: Span) {
...
pub fn walk_ternary<V: AstVisitor + ?Sized>(v: &mut V, cond: &SExpr, then_: &SExpr, else_: Option<&SExpr>, span: Span) {
...
pub fn walk_coalesce<V: AstVisitor + ?Sized>(v: &mut V, expr: &SExpr, default: &SExpr, _span: Span) {
  v.visit_expr(&expr.node, expr.span);
  v.visit_expr(&default.node, default.span);
}

pub fn walk_slice<V: AstVisitor + ?Sized>(v: &mut V, expr: &SExpr, start: Option<&SExpr>, end: Option<&SExpr>, _span: Span) {
  v.visit_expr(&expr.node, expr.span);
  if let Some(s) = start {
...
pub fn walk_loop<V: AstVisitor + ?Sized>(v: &mut V, stmts: &[SStmt], span: Span) {
  for s in stmts {
    v.visit_stmt(&s.node, s.span);
  }
...
pub fn walk_assert<V: AstVisitor + ?Sized>(v: &mut V, expr: &SExpr, msg: Option<&SExpr>, _span: Span) {
  v.visit_expr(&expr.node, expr.span);
  if let Some(m) = msg {
...
pub fn walk_par<V: AstVisitor + ?Sized>(v: &mut V, stmts: &[SStmt], span: Span) {
  for s in stmts {
    v.visit_stmt(&s.node, s.span);
  }
...
pub fn walk_sel<V: AstVisitor + ?Sized>(v: &mut V, arms: &[SelArm], span: Span) {
  for arm in arms {
...
pub fn walk_with_resource<V: AstVisitor + ?Sized>(v: &mut V, resources: &[(SExpr, String)], body: &[SStmt], _span: Span) {
...
pub fn walk_with_context<V: AstVisitor + ?Sized>(v: &mut V, fields: &[(String, SExpr)], body: &[SStmt], _span: Span) {
