use crate::ast::{BindTarget, Binding, McpOutputType, SStmt, Stmt, UseKind, UseStmt};
use crate::span::Span;
...
impl Checker {
    ...
    pub(super) fn check_stmt(&mut self, stmt: &SStmt) -> Type {
        match &stmt.node {
            ...
            Stmt::TypeDef { name, variants, .. } => {
                ...
                let variant_types: Vec<types::Variant> = variants
                    .iter()
                    .map(|(n, arity)| types::Variant {
                        name: n.clone(),
                        fields: vec![Type::Unknown; *arity],
                    })
                    .collect();
            ...
            Stmt::McpDecl { tools, .. } => {
                for tool in tools {
                    for f in &tool.input {
                        let _ = super::named_to_type(&f.type_name);
                    }
                    let _ = resolve_mcp_output(&tool.output);
                }
                Type::Unit
            }
            Stmt::TraitDecl { name, entries, .. } => {
                let fields: Vec<(String, Type)> = entries
                    .iter()
                    .filter_map(|e| {
                        if let crate::ast::TraitEntry::Field(f) = e {
                            Some((f.name.clone(), super::named_to_type(&f.type_name)))
                        } else {
                            None
                        }
                    })
                    .collect();
                if !fields.is_empty() {
                    self.trait_fields.insert(name.clone(), fields);
                }
    ...
    fn check_import_conflict(&mut self, name: &str, span: Span) {
        if let Some(existing) = self.import_sources.get(name) {
            self.emit_warning(
                format!("'{name}' already imported at offset {}", existing.offset),
                span,
            );
        } else {
