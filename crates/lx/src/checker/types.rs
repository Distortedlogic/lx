  ...
  pub fn resolve_deep(&self, ty: &Type) -> Type {
    match ty {
      ...
      Type::Record(fields) => Type::Record(
        fields.iter().map(|(n, t)| (n.clone(), self.resolve_deep(t))).collect(),
      ),
  ...
  pub fn unify(&mut self, a: &Type, b: &Type) -> Result<Type, String> {
    ...
    match (&a, &b) {
      ...
      (Type::Tuple(a_elems), Type::Tuple(b_elems)) if a_elems.len() == b_elems.len() => {
        let elems: Result<Vec<_>, _> = a_elems.iter().zip(b_elems.iter())
      ...
      _ => Err(format!("type mismatch: expected {}, got {}", display(&a), display(&b))),
    }
  }
...
pub fn display(ty: &Type) -> String {
  match ty {
    ...
    Type::Record(fields) => {
      let fs: Vec<_> = fields.iter().map(|(n, t)| format!("{n}: {}", display(t))).collect();
      format!("{{{}}}", fs.join("  "))
    },
    Type::Tuple(elems) => {
