...
use crate::ast::{Expr, ListElem, SExpr, SStmt, Stmt};
...
fn collect_free(expr: &Expr, vars: &mut HashSet<String>, bound: &mut HashSet<String>) {
    match expr {
        ...
        Expr::AgentSend { target, msg } | Expr::AgentAsk { target, msg } => {
            collect_free(&target.node, vars, bound);
            collect_free(&msg.node, vars, bound);
        }
        _ => {}
    }
}
