use crate::ast::{
    BinOp, Expr, Literal, MapEntry, MatchArm, Param, Pattern, SExpr, SPattern, SStmt, SType,
};
...
impl Checker {
    ...
    pub(super) fn check_mutable_captures_stmts(
        &mut self,
        stmts: &[SStmt],
    ...
    pub(super) fn synth_apply(&mut self, func: &SExpr, arg: &SExpr) -> Type {
        if let (Expr::Ident(name) | Expr::TypeConstructor(name), Expr::Record(rec_fields)) =
            (&func.node, &arg.node)
                ...
                for rf in rec_fields {
                    if rf.name.as_deref() == Some(trait_name.as_str()) {
                        ...
                        if let Err(msg) = self.table.unify(trait_type, &val_t) {
                            self.emit(
                                format!("Trait field `{trait_name}` type mismatch: {msg}"),
                                rf.value.span,
                            );
                        }
                    }
    ...
    pub(super) fn synth_map(&mut self, entries: &[MapEntry]) -> Type {
        if entries.is_empty() {
            return Type::Map {
                key: Box::new(self.fresh()),
                value: Box::new(self.fresh()),
            };
        }
