...
impl super::Parser {
  pub(super) fn parse_infix(&mut self, left: SExpr, kind: &TokenKind, rbp: u8) -> Result<SExpr, LxError> {
    ...
    self.skip_semis();
    match kind {
      TokenKind::Pipe => self.parse_binary_infix(left, rbp, |l, r| Expr::Pipe { left: l, right: r }, start),
      TokenKind::QQ => self.parse_binary_infix(left, rbp, |l, r| Expr::Coalesce { expr: l, default: r }, start),
