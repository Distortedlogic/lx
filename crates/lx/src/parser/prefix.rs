use crate::ast::{SExpr, Expr, Literal, UnaryOp, StrPart, ListElem, RecordField, MapEntry, SStmt, ShellMode, SelArm};
    ...
    match tok.kind {
      ...
      TokenKind::With => {
        ...
        let name = match self.peek().clone() {
          ...
          _ => return Err(LxError::parse("expected name after 'with'", self.tokens[self.pos].span, None)),
        };
        ...
        let end = self.expect_kind(&TokenKind::RBrace)?.span.end();
        Ok(SExpr::new(Expr::With { name, value: Box::new(value), body, mutable }, Span::from_range(tok.span.offset, end)))
      },
