...
impl super::Parser {
  pub(crate) fn parse_prefix(&mut self) -> Result<SExpr, LxError> {
    ...
    match tok.kind {
      ...
      TokenKind::Emit => {
        ...
        Ok(SExpr::new(Expr::Emit { value: Box::new(value) }, Span::from_range(tok.span.offset(), end)))
      },
      TokenKind::Yield => {
        let value = self.parse_expr(0)?;
        let end = value.span.end();
        Ok(SExpr::new(Expr::Yield { value: Box::new(value) }, Span::from_range(tok.span.offset(), end)))
      },
  ...
  fn parse_unary(&mut self, op: UnaryOp, start: usize) -> Result<SExpr, LxError> {
  ...
  fn parse_shell(&mut self, mode: ShellMode, start: usize) -> Result<SExpr, LxError> {
  ...
  pub(crate) fn parse_string(&mut self, start: usize) -> Result<SExpr, LxError> {
    let mut parts = Vec::new();
    loop {
      match self.peek().clone() {
