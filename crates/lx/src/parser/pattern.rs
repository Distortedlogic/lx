...
impl super::Parser {
    pub(crate) fn parse_pattern(&mut self) -> Result<SPattern, LxError> {
        ...
        match tok.kind {
            ...
            TokenKind::True => Ok(SPattern::new(
                Pattern::Literal(Literal::Bool(true)),
                tok.span,
            )),
            TokenKind::False => Ok(SPattern::new(
                Pattern::Literal(Literal::Bool(false)),
                tok.span,
            )),
            TokenKind::RawStr(s) => Ok(SPattern::new(
                Pattern::Literal(Literal::RawStr(s)),
                tok.span,
            )),
            TokenKind::Minus => {
                let next = self.advance().clone();
                match next.kind {
                    TokenKind::Int(n) => Ok(SPattern::new(
                        Pattern::Literal(Literal::Int(-n)),
                        Span::from_range(tok.span.offset, next.span.end()),
                    )),
                    TokenKind::Float(f) => Ok(SPattern::new(
        ...
        loop {
            match self.peek().clone() {
                ...
                TokenKind::StrEnd => {
                    ...
                    return Ok(SPattern::new(
                        Pattern::Literal(Literal::Str(parts)),
    ...
    fn parse_constructor_pattern(
        ...
        Ok(SPattern::new(
            ...
            Span::from_range(name_span.offset, end),
        ))
    }
    ...
    pub(super) fn expr_to_pattern(&self, expr: &SExpr) -> Result<SPattern, LxError> {
        ...
        match &expr.node {
            ...
            _ => Err(LxError::parse(
                "expected pattern on left side of '='",
                span,
                None,
            )),
        }
