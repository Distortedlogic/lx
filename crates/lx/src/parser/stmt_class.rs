...
) -> impl Parser<'a, I, (Vec<Sym>, Vec<ClassField>, Vec<AgentMethod>), extra::Err<Rich<'a, TokenKind, Span>>> + Clone
where
  I: ValueInput<'a, Token = TokenKind, Span = Span>,
{
  let uses_member = just(TokenKind::Use).ignore_then(type_name()).map(ClassMember::Uses);
  ...
  just(TokenKind::LBrace)
    ...
    .map(|members| {
      ...
      for m in members {
        match m {
          ClassMember::Uses(name) => uses.push(name),
