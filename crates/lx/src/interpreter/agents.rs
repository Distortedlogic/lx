...
use crate::ast::{SExpr, ProtocolField, McpToolDecl, McpOutputType};
use crate::error::LxError;
use crate::span::Span;
use crate::value::{Value, ProtoFieldDef, McpToolDef, McpOutputDef};
...
impl Interpreter {
  ...
  pub(super) fn eval_agent_send(&mut self, target_expr: &SExpr, msg_expr: &SExpr, span: Span) -> Result<Value, LxError> {
  ...
  pub(super) fn eval_agent_ask(&mut self, target_expr: &SExpr, msg_expr: &SExpr, span: Span) -> Result<Value, LxError> {
    let target = self.eval(target_expr)?;
    let msg = self.eval(msg_expr)?;
    if let Value::Record(ref fields) = target
      && let Some(pid_val) = fields.get("__pid") {
  ...
  pub(super) fn eval_protocol_def(&mut self, name: &str, fields: &[ProtocolField], span: Span) -> Result<Value, LxError> {
  ...
  pub(super) fn eval_mcp_decl(&mut self, name: &str, tools: &[McpToolDecl], _span: Span) -> Result<Value, LxError> {
    ...
    for t in tools {
      ...
      for f in &t.input {
        ...
        input.push(ProtoFieldDef { name: f.name.clone(), type_name: f.type_name.clone(), default });
      }
      let output = self.resolve_mcp_output(&t.output);
      tool_defs.push(McpToolDef { name: t.name.clone(), input, output });
    }
    let val = Value::McpDecl { name: Arc::from(name), tools: Arc::new(tool_defs) };
    let mut env = self.env.child();
    env.bind(name.to_string(), val);
    self.env = env.into_arc();
    Ok(Value::Unit)
    ...
    match out {
      ...
      McpOutputType::Record(fields) => {
        let defs = fields.iter()
          .map(|f| ProtoFieldDef { name: f.name.clone(), type_name: f.type_name.clone(), default: None })
          .collect();
  ...
  pub(super) fn apply_protocol(&mut self, name: &str, fields: &Arc<Vec<ProtoFieldDef>>, arg: &Value, span: Span) -> Result<Value, LxError> {
  ...
  pub(super) fn apply_mcp_decl(&mut self, name: &str, tools: &Arc<Vec<McpToolDef>>, client: &Value, span: Span) -> Result<Value, LxError> {
    let Value::Record(rec) = client else {
      return Err(LxError::type_err(
        format!("MCP {name}: expected connection Record, got {}", client.type_name()),
        span,
      ));
    };
    let mcp_id: u64 = rec.get("__mcp_id")
      .and_then(|v| v.as_int())
      .and_then(|n| n.try_into().ok())
  ...
  pub(super) fn update_record_field(val: &Value, fields: &[String], new_val: Value, span: Span) -> Result<Value, LxError> {
    match (val, fields) {
      ...
      (Value::Record(rec), [field, rest @ ..]) => {
        let inner = rec.get(field).ok_or_else(||
          LxError::runtime(format!("field '{field}' not found"), span))?;
      ...
      (other, _) => Err(LxError::type_err(
        format!("field update requires Record, got {}", other.type_name()), span)),
    }
  }
}
