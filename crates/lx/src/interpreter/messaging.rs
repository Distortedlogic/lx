...
impl Interpreter {
  pub(super) async fn eval_tell(&mut self, target: ExprId, msg: ExprId, span: SourceSpan) -> EvalResult<LxVal> {
    ...
    fields.insert(crate::sym::intern("from"), LxVal::str(agent_name));
    fields.insert(crate::sym::intern("to"), LxVal::str(&target_name));
    fields.insert(crate::sym::intern("msg"), msg_val);
  ...
  pub(super) async fn eval_ask(&mut self, target: ExprId, msg: ExprId, span: SourceSpan) -> EvalResult<LxVal> {
    ...
    let agent_name = self.agent_name.as_deref().unwrap_or("main");
    let mut fields = IndexMap::new();
    fields.insert(crate::sym::intern("ask_id"), LxVal::int(ask_id));
    fields.insert(crate::sym::intern("from"), LxVal::str(agent_name));
    fields.insert(crate::sym::intern("to"), LxVal::str(&target_name));
    fields.insert(crate::sym::intern("msg"), msg_val);
    ...
    reply_fields.insert(crate::sym::intern("ask_id"), LxVal::int(ask_id));
    reply_fields.insert(crate::sym::intern("from"), LxVal::str(&target_name));
    reply_fields.insert(crate::sym::intern("to"), LxVal::str(agent_name));
    reply_fields.insert(crate::sym::intern("msg"), result.clone());
