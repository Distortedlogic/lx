        ...
        for f in fields {
            if f.is_spread {
                ...
                match v {
                    ...
                    other => {
                        return Err(LxError::type_err(
                            format!("spread requires Record, got {}", other.type_name()),
                            f.value.span,
                        ));
                    }
                }
                ...
                let name = f.name.clone().unwrap_or_else(|| {
                    if let Expr::Ident(n) = &f.value.node {
                        n.clone()
                    } else {
                        "_".into()
                    }
        ...
        for entry in entries {
            if entry.is_spread {
                ...
                match v {
                    ...
                    other => {
                        return Err(LxError::type_err(
                            format!("spread requires Map, got {}", other.type_name()),
                            entry.value.span,
                        ));
                    }
                }
                ...
                let key_expr = entry
                    .key
                    .as_ref()
                    .expect("non-spread map entry must have a key");
