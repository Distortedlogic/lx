...
use crate::ast::{SExpr, Literal, BinOp, UnaryOp, StrPart, SelArm, SStmt};
...
impl Interpreter {
  ...
  pub(super) fn eval_binary(&mut self, op: &BinOp, left: &SExpr, right: &SExpr, span: Span) -> Result<Value, LxError> {
    if *op == BinOp::And {
  ...
  pub(super) fn binary_op(&self, op: &BinOp, lv: &Value, rv: &Value, span: Span) -> Result<Value, LxError> {
    match op {
  ...
  pub(super) fn eval_unary(&mut self, op: &UnaryOp, operand: &SExpr, span: Span) -> Result<Value, LxError> {
  ...
  pub(super) fn eval_slice(&mut self, expr: &SExpr, start: Option<&SExpr>, end: Option<&SExpr>, span: Span) -> Result<Value, LxError> {
  ...
  pub(super) fn eval_assert(&mut self, expr: &SExpr, msg: &Option<Box<SExpr>>, span: Span) -> Result<Value, LxError> {
    let val = self.eval(expr)?;
