...
impl super::Interpreter {
  pub(super) fn eval_match(&mut self, scrutinee: &SExpr, arms: &[MatchArm], span: Span) -> Result<Value, LxError> {
  ...
  pub(super) fn try_match_pattern(&self, pattern: &Pattern, value: &Value) -> Option<Vec<(String, Value)>> {
    match pattern {
      ...
      Pattern::Record { fields, rest } => {
        ...
        if let Some(rest_name) = rest {
          ...
          let remaining: indexmap::IndexMap<String, Value> =
