...
use crate::ast::{Program, Stmt, BindTarget, UseStmt, UseKind};
...
impl Interpreter {
  pub(super) fn eval_use(&mut self, use_stmt: &UseStmt, span: Span) -> Result<(), LxError> {
    ...
    match &use_stmt.kind {
      UseKind::Whole => {
        let module_name = use_stmt.path.last()
      ...
      UseKind::Selective(names) => {
        for name in names {
          let val = exports.bindings.get(name)
            .ok_or_else(|| LxError::runtime(format!("'{name}' not exported by module"), span))?;
          env.bind(name.clone(), val.clone());
        }
      },
    }
  ...
  fn load_module(&mut self, file_path: &PathBuf, span: Span) -> Result<ModuleExports, LxError> {
    ...
    let source = std::fs::read_to_string(file_path)
      .map_err(|e| LxError::runtime(format!("cannot read module '{}': {e}", file_path.display()), span))?;
