...
fn bi_h1(args: &[Value], span: Span, _ctx: &Arc<RuntimeCtx>) -> Result<Value, LxError> {
    heading(1, args, span)
}
fn bi_h2(args: &[Value], span: Span, _ctx: &Arc<RuntimeCtx>) -> Result<Value, LxError> {
    heading(2, args, span)
}
fn bi_h3(args: &[Value], span: Span, _ctx: &Arc<RuntimeCtx>) -> Result<Value, LxError> {
    heading(3, args, span)
}
...
fn bi_para(args: &[Value], span: Span, _ctx: &Arc<RuntimeCtx>) -> Result<Value, LxError> {
    ...
    Ok(node_rec(
        "para",
        vec![("text", Value::Str(Arc::from(text)))],
    ))
}
...
fn bi_list(args: &[Value], span: Span, _ctx: &Arc<RuntimeCtx>) -> Result<Value, LxError> {
    ...
    Ok(node_rec(
        "list",
        vec![("items", Value::List(items.clone()))],
    ))
}
...
fn render_table(r: &IndexMap<String, Value>, out: &mut String) {
    let Some(Value::List(headers)) = r.get("headers") else {
        return;
    };
    let Some(Value::List(rows)) = r.get("rows") else {
        return;
    };
    out.push('|');
