...
pub fn object_insert(fields: indexmap::IndexMap<String, crate::value::LxVal>) -> u64 {
...
fn update_nested_record(
    val: &crate::value::LxVal,
    path: &[String],
    new_val: crate::value::LxVal,
) -> Result<crate::value::LxVal, String> {
    ...
    match path {
        [field] => {
            ...
            Ok(crate::value::LxVal::Record(Arc::new(new_rec)))
        }
        [field, rest @ ..] => {
            let inner = rec
                .get(field)
...
fn bi_to_record(args: &[LxVal], span: Span, _ctx: &Arc<RuntimeCtx>) -> Result<LxVal, LxError> {
    ...
    let s = STORES
        .get(&id)
        .ok_or_else(|| LxError::runtime("store: not found", span))?;
    let fields: indexmap::IndexMap<String, LxVal> =
