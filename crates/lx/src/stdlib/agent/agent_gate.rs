...
fn bi_gate(args: &[Value], span: Span, ctx: &Arc<RuntimeCtx>) -> Result<Value, LxError> {
    let name = args[0]
        .as_str()
        .ok_or_else(|| LxError::type_err("agent.gate: first arg must be gate name (Str)", span))?;
    let config = &args[1];
    ...
    match &response {
        Value::Record(r) => {
            let approved = r.get("approved").and_then(|v| v.as_bool()).unwrap_or(false);
            let approver = r
                .get("approver")
                .and_then(|v| v.as_str())
                .unwrap_or("unknown")
            ...
            result.insert("approver".into(), Value::Str(Arc::from(approver.as_str())));
            result.insert("reason".into(), Value::Str(Arc::from(reason.as_str())));
        ...
        _ => handle_timeout_policy(name, &on_timeout, &now),
    }
}

fn handle_timeout_policy(name: &str, on_timeout: &str, now: &str) -> Result<Value, LxError> {
    match on_timeout {
        "approve" => {
            let mut result = IndexMap::new();
            result.insert("approved".into(), Value::Bool(true));
            result.insert("approver".into(), Value::Str(Arc::from("auto_timeout")));
            result.insert(
                "reason".into(),
                Value::Str(Arc::from("auto-approved on timeout")),
            );
...
fn parse_gate_config(config: &Value, span: Span) -> Result<(Option<Value>, i64, String), LxError> {
    match config {
