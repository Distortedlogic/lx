...
fn keyword_overlap(haystack: &str, keywords_source: &str, min_word_len: usize) -> (usize, usize) {
    let haystack_lower = haystack.to_lowercase();
    let keywords: Vec<String> = keywords_source.split_whitespace()
        .filter(|w| w.len() > min_word_len).map(|w| w.to_lowercase()).collect();
...
pub(super) fn assemble_result(
    ...
    for (name, score) in kept {
        let weight = rubric
            ...
            .map(|c| c.weight)
            .unwrap_or(1);
        all_cats.push(make_eval_category(
            name,
            *score,
            true,
            "previously passed",
    ...
    if let Some(cats) = jv.get("categories").and_then(|v| v.as_array()) {
        for cat in cats {
            ...
            let weight = rubric
                ...
                .unwrap_or(1);
            let passed = score >= 70;
            all_cats.push(make_eval_category(name, score, passed, fb));
            total_score += score * weight;
            total_weight += weight;
            if !passed {
...
pub(super) fn bi_quick_grade(
    ...
    for cat in &fields.rubric {
        let (hits, total) = keyword_overlap(&fields.work, &cat.description, 3);
        ...
        let passed = score >= 50;
        categories.push(make_eval_category(
            &cat.name,
            score,
            passed,
            if passed {
