...
fn bi_sleep(args: &[Value], span: Span) -> Result<Value, LxError> {
    let ms = match &args[0] {
        ...
        _ => return Err(LxError::type_err("time.sleep expects Int or Float ms", span)),
    };
...
fn bi_parse(args: &[Value], span: Span) -> Result<Value, LxError> {
    ...
    match DateTime::parse_from_str(input, fmt) {
        Ok(dt) => Ok(Value::Ok(Box::new(timestamp_to_record(dt.with_timezone(&Utc))))),
        Err(e) => Ok(Value::Err(Box::new(Value::Str(
            Arc::from(format!("time.parse: {e}").as_str()),
        )))),
    }
}
...
fn timestamp_to_record(dt: DateTime<Utc>) -> Value {
    ...
    fields.insert("iso".into(), Value::Str(Arc::from(dt.to_rfc3339().as_str())));
    let local: DateTime<Local> = dt.with_timezone(&Local);
    fields.insert("local".into(), Value::Str(Arc::from(local.to_rfc3339().as_str())));
