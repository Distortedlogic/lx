...
pub(super) fn visit_expr_diag(w: &mut Walker, expr: &Expr, span: Span) {
    match expr {
        Expr::AgentSend { target, msg } => {
            ...
            w.add_edge_typed(&ctx, &to, extract_msg_label(&msg.node), "dashed", "agent");
        }
        Expr::AgentAsk { target, msg } => {
            let to = resolve_target(w, &target.node);
            let ctx = w.context.clone();
            w.add_edge_typed(&ctx, &to, extract_msg_label(&msg.node), "solid", "agent");
        }
        Expr::StreamAsk { target, msg } => {
            let to = resolve_target(w, &target.node);
            let ctx = w.context.clone();
            w.add_edge_typed(&ctx, &to, extract_msg_label(&msg.node), "dotted", "stream");
        }
        Expr::Par(stmts) => {
            let fork_id = w.add_node_at("fork", "par".into(), "fork", Some(span));
            let ctx = w.context.clone();
        ...
        Expr::Sel(arms) => {
            let dec_id = w.add_node_at("sel", "sel".into(), "decision", Some(span));
            let ctx = w.context.clone();
        ...
        Expr::Match { scrutinee, arms } => {
            ...
            let dec_id = w.add_node_at("match", label, "decision", Some(span));
            let ctx = w.context.clone();
            w.add_edge(&ctx, &dec_id, String::new(), "solid");
            let saved = w.context.clone();
            w.context = dec_id;
        ...
        Expr::Ternary { cond, then_, else_ } => {
            ...
            let dec_id = w.add_node_at("cond", label, "decision", Some(span));
            let ctx = w.context.clone();
            w.add_edge(&ctx, &dec_id, String::new(), "solid");
            let saved = w.context.clone();
        ...
        Expr::Loop(stmts) => {
            let loop_id = w.add_node_at("loop", "loop".into(), "loop", Some(span));
            let ctx = w.context.clone();
        ...
        Expr::Refine {
            ...
            let refine_id = w.add_node_at("loop", "refine".into(), "loop", Some(span));
            let ctx = w.context.clone();
    ...
    match (module, method) {
        ("agent", "kill") => {
            if let Some(first) = args.first() {
                ...
                let ctx = w.context.clone();
                w.add_edge_typed(&ctx, &target, "kill".into(), "dashed", "agent");
            }
            return;
        }
        ("agent", "dispatch") => {
            let id = w.add_node_at("fork", "dispatch".into(), "fork", Some(span));
            let ctx = w.context.clone();
            w.add_edge(&ctx, &id, String::new(), "solid");
            if let Some(first) = args.first()
        ...
        ("agent", "spawn") => {
            ...
            let id = w.add_node_at("agent", label, "agent", Some(span));
            let ctx = w.context.clone();
            w.add_edge_typed(&ctx, &id, String::new(), "solid", "agent");
            return;
        }
        ("mcp", "call") => {
            if let Some(first) = args.first()
