...
pub fn build() -> IndexMap<String, LxVal> {
    ...
    m.insert(
        "to_graph_chart".into(),
        mk("diag.to_graph_chart", 1, bi_to_graph_chart),
    );
    m
}
...
pub fn graph_to_echart_json(graph: &Graph) -> String {
    ...
    let mut layer_positions: std::collections::HashMap<usize, usize> =
    ...
    let nodes_json: Vec<serde_json::Value> = graph
        ...
        .map(|n| {
            ...
            let y_pos = *pos;
            *pos += 1;
            let symbol_size = if matches!(n.kind.as_str(), "fork" | "join") {
                serde_json::json!([40, 10])
            } else {
                serde_json::json!(40)
            };
            serde_json::json!({
    ...
    let edges_json: Vec<serde_json::Value> = graph
        ...
        .map(|e| {
            ...
            if !e.label.is_empty() {
                edge["label"] =
                    serde_json::json!({"show": true, "formatter": e.label, "fontSize": 10});
            }
            edge
        })
        .collect();
