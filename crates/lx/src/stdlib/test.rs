...
pub fn build() -> IndexMap<String, Value> {
    ...
    m.insert(
        "run_scenario".into(),
        mk("test.run_scenario", 2, test_run::bi_run_scenario),
    );
    m.insert(
        "report".into(),
        mk("test.report", 1, test_report::bi_report),
    );
    m
}
...
fn bi_spec(args: &[Value], span: Span, _ctx: &Arc<RuntimeCtx>) -> Result<Value, LxError> {
    ...
    let flow = opts
        .get("flow")
        .ok_or_else(|| LxError::type_err("test.spec: 'flow' is required", span))?
        .clone();
    ...
    let grader = opts
        .get("grader")
        .ok_or_else(|| LxError::type_err("test.spec: 'grader' is required", span))?
        .clone();
...
fn bi_scenario(args: &[Value], span: Span, _ctx: &Arc<RuntimeCtx>) -> Result<Value, LxError> {
    ...
    let scenario_name = args[1]
        .as_str()
        .ok_or_else(|| LxError::type_err("test.scenario: name must be Str", span))?;
    let opts = extract_record(&args[2], "test.scenario", span)?;

    let input = opts
        .get("input")
        .ok_or_else(|| LxError::type_err("test.scenario: 'input' is required", span))?
        .clone();
