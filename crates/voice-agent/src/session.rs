...
pub async fn handle_session(ws: WebSocket, agent: Arc<dyn AgentBackend>) {
  ...
  let mut session = VoiceSession { id: uuid::Uuid::new_v4(), state: SessionState::Idle };
  ...
  match whisper_client::WHISPER.health().await {
    Ok(true) => {},
    Ok(false) => {
      let _ = tx.send(ServerMessage::Error { message: "Whisper service is not reachable".into() }).await;
    },
    Err(e) => {
      let _ = tx.send(ServerMessage::Error { message: format!("Whisper health check failed: {e}") }).await;
    },
  }
  match kokoro_client::KOKORO.health().await {
    Ok(true) => {},
    Ok(false) => {
      let _ = tx.send(ServerMessage::Error { message: "Kokoro TTS service is not reachable".into() }).await;
    },
    Err(e) => {
      let _ = tx.send(ServerMessage::Error { message: format!("Kokoro health check failed: {e}") }).await;
    },
  }
  ...
  match session.state {
    SessionState::Standby => {
      ...
      if detector.should_check() {
        match detector.check_trigger(detector::TRIGGER_ACTIVATE, &whisper_client::WHISPER).await {
          Ok(true) => {
    ...
    SessionState::Activated => {
      ...
      let trigger_fired = if command_buffer.len() >= MAX_COMMAND_BYTES {
        ...
        detector.check_trigger(detector::TRIGGER_RESPOND, &whisper_client::WHISPER).await.unwrap_or(false)
      } else {
        false
      };
...
async fn process_command(audio: Vec<u8>, agent: Arc<dyn AgentBackend>, tx: mpsc::Sender<ServerMessage>, cancel: CancellationToken) -> anyhow::Result<()> {
  ...
  tx.send(ServerMessage::TextTranscript { text: clean_text.clone() }).await.map_err(|e| anyhow::anyhow!("channel closed: {e}"))?;

  if clean_text.is_empty() {
    return Ok(());
  }

  let response = tokio::time::timeout(tokio::time::Duration::from_secs(120), agent.query(&clean_text))
    .await
  ...
  tx.send(ServerMessage::AgentResponse { text: response.clone() }).await.map_err(|e| anyhow::anyhow!("channel closed: {e}"))?;
  ...
  for (seq, chunk) in chunks.into_iter().enumerate() {
    ...
    tx.send(ServerMessage::AudioResponse { data, seq: seq as u64 }).await.map_err(|e| anyhow::anyhow!("channel closed: {e}"))?;
  }
  Ok(())
}
