namespace LxCharts {
  var AXIS_FORMATTERS: Record<string, Function> = {
  ...
  function applyFormatters(opts: any, el: HTMLElement): void {
    ...
    var extra: any = ds.extra && ds.extra.length > 0 ? JSON.parse(ds.extra) : null;
    if (ds.tooltipFmt) {
      ...
      switch (ds.tooltipFmt) {
        ...
        case 'scatter':
          if (extra) {
            opts.tooltip.formatter = function(params: any) {
    ...
    if (ds.labelFmt) {
      if (opts.series) {
        ...
        for (var i = 0; i < sarr.length; i++) {
          if (!sarr[i].label) sarr[i].label = {};
          switch (ds.labelFmt) {
            case 'alpsPopulation':
              if (extra) {
                var popData = extra.pop;
                sarr[i].label.formatter = function(params: any) {
  ...
  function themeAxis(axisOpt: any, fg: string, bc: string, sc: string): void {
    if (!axisOpt) return;
    var arr = Array.isArray(axisOpt) ? axisOpt : [axisOpt];
    for (var i = 0; i < arr.length; i++) {
      var ax = arr[i];
      ...
      if (ax.axisLabel.textBorderWidth === undefined) ax.axisLabel.textBorderWidth = 0;
      if (!ax.nameTextStyle) ax.nameTextStyle = {};
      if (!ax.nameTextStyle.color) ax.nameTextStyle.color = fg;
      if (ax.nameTextStyle.textBorderWidth === undefined) ax.nameTextStyle.textBorderWidth = 0;
    }
  }
  export function initChart(id: string, opts: any): void {
    ...
    if (opts.title) { var t = Array.isArray(opts.title) ? opts.title[0] : opts.title; if (t) { if (!t.textStyle) t.textStyle = {}; if (!t.textStyle.color) t.textStyle.color = fg; } var g = Array.isArray(opts.grid) ? opts.grid[0] : opts.grid; if (g && !g.top) g.top = 35; }
    applyFormatters(opts, el);
    if (!opts.tooltip.valueFormatter) opts.tooltip.valueFormatter = LxCharts.formatNumber;
