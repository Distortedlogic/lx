...
fn highest_notification_level(state: &TabsState, tab_id: &str) -> Option<NotificationLevel> {
    ...
    tab.root
        .all_pane_ids()
        .into_iter()
        .filter_map(|id| state.get_notification(&id).map(|n| n.level))
        .max()
}
