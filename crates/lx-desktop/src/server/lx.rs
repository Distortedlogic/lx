...
impl Write for PtySessionWriter {
    fn write(&mut self, buf: &[u8]) -> io::Result<usize> {
        self.rt
            .block_on(self.session.send_input(buf.to_vec()))
...
fn make_writer_factory(
    run_id: String,
    source_dir: String,
    spawn_tx: Option<SpawnSender>,
) -> WriterFactory {
    let rt = tokio::runtime::Handle::current();
    Box::new(
        move |agent_id: &str, agent_name: &str| -> Box<dyn Write + Send> {
            let terminal_id = format!("lx-{run_id}-{agent_id}");

            if let Some(ref tx) = spawn_tx {
                let pane = PaneNode::Terminal {
                    id: terminal_id.clone(),
                    working_dir: source_dir.clone(),
                    command: None,
                };
                let title = format!("lx: {agent_name}");
                let req = crate::layout::shell::TerminalSpawnRequest {
                    id: terminal_id.clone(),
                    title,
                    pane,
                };
            ...
            match pty_session::get_or_create(&terminal_id, 120, 40, Some(&source_dir), None) {
                Ok(sess) => Box::new(PtySessionWriter {
                    session: sess,
                    rt: rt.clone(),
                }),
                Err(e) => {
