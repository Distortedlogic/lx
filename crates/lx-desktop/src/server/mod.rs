...
impl ServerState {
  pub fn new() -> Self {
    ...
    Self { settings: RwLock::new(settings), activity: RwLock::new(VecDeque::new()), settings_path }
  }
}
...
pub fn router() -> Router {
  ...
  Router::new().route("/api/health", get(health)).merge(settings_api::routes()).merge(activity_api::routes()).with_state(state)
}
