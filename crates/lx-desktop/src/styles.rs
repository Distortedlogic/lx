...
pub const BTN_OUTLINE_SM: &str =
  "border border-[var(--outline-variant)] text-[var(--on-surface)] rounded px-3 py-1.5 text-xs hover:bg-[var(--surface-container-high)] transition-colors";
...
pub const INPUT_FIELD: &str =
  "w-full rounded border border-[var(--outline-variant)] px-2.5 py-1.5 bg-transparent outline-none text-sm font-mono placeholder:text-[var(--outline)]/40";
