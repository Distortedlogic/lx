    ...
    pub fn close_tab(&mut self, tab_id: &str) {
        ...
        if was_active {
            let new_active = if let Some(i) = idx {
                if i < self.tabs.len() { Some(&self.tabs[i]) } else { self.tabs.last() }
            } else {
                self.tabs.first()
            };
            self.active_tab_id = new_active.map(|t| t.id.clone());
            self.focused_pane_id =
                new_active.and_then(|t| t.root().first_leaf().map(|l| l.pane_id().to_owned()));
        }
    }
    ...
    pub fn close_pane_in_active_tab(&mut self, pane_id: &str) {
        ...
        if let Some(new_root) = root.close(pane_id) {
            if self.focused_pane_id.as_deref() == Some(pane_id) {
                self.focused_pane_id =
                    new_root.first_leaf().map(|l| l.pane_id().to_owned());
            }
