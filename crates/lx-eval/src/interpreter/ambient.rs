...
fn bi_context_current(_args: &[LxVal], _span: SourceSpan, _ctx: &dyn lx_value::BuiltinCtx) -> Result<LxVal, LxError> {
  let fields = AMBIENT_SNAPSHOT.with(|s| s.borrow().clone());
  Ok(LxVal::record(fields))
}

fn bi_context_get(args: &[LxVal], span: SourceSpan, _ctx: &dyn lx_value::BuiltinCtx) -> Result<LxVal, LxError> {
