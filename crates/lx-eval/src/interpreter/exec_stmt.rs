...
use lx_ast::ast::{BindTarget, Stmt, StmtId, StmtTypeDef};
use lx_span::sym::Sym;
use lx_value::Env;
use lx_value::{EvalResult, LxError};
use lx_value::{LxClass, LxTrait, LxVal};
      ...
      Stmt::TraitDecl(data) => {
        ...
        for m in &data.methods {
          ...
          for f in &m.input {
            ...
            input.push(lx_value::FieldDef { name: f.name, type_name: f.type_name, default, constraint: None });
          }
          method_defs.push(lx_value::TraitMethodDef { name: m.name, input, output: m.output });
        }
