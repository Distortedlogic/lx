...
use lx_ast::ast::{AstArena, ExprId, Param};
use lx_span::sym::Sym;
use lx_value::Env;
use lx_value::{BuiltinKind, LxFunc, LxVal};
use lx_value::{EvalResult, EvalSignal, LxError};
  ...
  pub async fn apply_func(&mut self, func: LxVal, arg: LxVal, span: SourceSpan) -> EvalResult<LxVal> {
    match func {
      ...
      LxVal::Class(c) => {
        ...
        Ok(LxVal::Object(Box::new(lx_value::LxObject { class_name: c.name, id, traits: c.traits, methods: c.methods })))
      },
