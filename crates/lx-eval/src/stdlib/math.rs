...
pub fn build() -> IndexMap<lx_span::sym::Sym, LxVal> {
  ...
  m.insert(lx_span::sym::intern("pi"), LxVal::Float(PI));
  m.insert(lx_span::sym::intern("e"), LxVal::Float(E));
  m.insert(lx_span::sym::intern("inf"), LxVal::Float(f64::INFINITY));
  m
}
...
fn bi_abs(args: &[LxVal], span: SourceSpan, _ctx: &dyn BuiltinCtx) -> Result<LxVal, LxError> {
  match &args[0] {
    LxVal::Int(n) => {
...
fn bi_ceil(args: &[LxVal], span: SourceSpan, _ctx: &dyn BuiltinCtx) -> Result<LxVal, LxError> {
  let f = to_f64(&args[0], span)?;
  Ok(LxVal::int(f.ceil() as i64))
}

fn bi_floor(args: &[LxVal], span: SourceSpan, _ctx: &dyn BuiltinCtx) -> Result<LxVal, LxError> {
  let f = to_f64(&args[0], span)?;
  Ok(LxVal::int(f.floor() as i64))
}

fn bi_round(args: &[LxVal], span: SourceSpan, _ctx: &dyn BuiltinCtx) -> Result<LxVal, LxError> {
  let f = to_f64(&args[0], span)?;
  Ok(LxVal::int(f.round() as i64))
}

fn bi_pow(args: &[LxVal], span: SourceSpan, _ctx: &dyn BuiltinCtx) -> Result<LxVal, LxError> {
  match (&args[0], &args[1]) {
...
fn bi_sqrt(args: &[LxVal], span: SourceSpan, _ctx: &dyn BuiltinCtx) -> Result<LxVal, LxError> {
  let f = to_f64(&args[0], span)?;
