...
pub(crate) fn bi_report(args: &[LxVal], span: SourceSpan, ctx: &Arc<dyn BuiltinCtx>) -> Result<LxVal, LxError> {
  ...
  let scenarios = if let Some(LxVal::List(list)) = results.get(&lx_span::sym::intern("scenarios")) { list.as_ref().clone() } else { Vec::new() };
  ...
  let spec_score = results.get(&lx_span::sym::intern("score")).and_then(|v| v.as_float()).unwrap_or(0.0);
  out.push_str(&format!("\nOverall: {spec_score:.2} — {passed_count}/{total} scenarios passed (threshold: {threshold:.2})\n"));

  println!("{out}");
  let mut fields = indexmap::IndexMap::new();
  fields.insert(lx_span::sym::intern("value"), LxVal::str(&out));
  ctx.event_stream().xadd("runtime/emit", "main", None, fields);
  Ok(LxVal::Unit)
}

fn format_scenario(sr: &IndexMap<lx_span::sym::Sym, LxVal>, out: &mut String, passed_count: &mut usize) {
  let s_name = sr.get(&lx_span::sym::intern("name")).and_then(|v| v.as_str()).unwrap_or("?");
  let s_score = sr.get(&lx_span::sym::intern("score")).and_then(|v| v.as_float()).unwrap_or(0.0);
  let s_passed = sr.get(&lx_span::sym::intern("passed")).and_then(|v| v.as_bool()).unwrap_or(false);
  let s_mean = sr.get(&lx_span::sym::intern("mean")).and_then(|v| v.as_float()).unwrap_or(s_score);
  let s_min = sr.get(&lx_span::sym::intern("min")).and_then(|v| v.as_float()).unwrap_or(s_score);
  let s_max = sr.get(&lx_span::sym::intern("max")).and_then(|v| v.as_float()).unwrap_or(s_score);
  let runs = if let Some(LxVal::List(l)) = sr.get(&lx_span::sym::intern("runs")) { l.len() } else { 0 };
  if s_passed {
    *passed_count += 1;
  }
  ...
  if let Some(LxVal::List(run_list)) = sr.get(&lx_span::sym::intern("runs"))
    && !run_list.is_empty()
  {
...
fn format_dimensions(run_list: &[LxVal], out: &mut String) {
  let mut dim_scores: IndexMap<lx_span::sym::Sym, Vec<f64>> = IndexMap::new();
  for run_val in run_list.iter() {
    if let LxVal::Record(rr) = run_val
      && let Some(LxVal::Record(scores)) = rr.get(&lx_span::sym::intern("scores"))
    {
