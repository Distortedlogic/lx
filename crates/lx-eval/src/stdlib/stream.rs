...
fn pull_next(id: u64, span: SourceSpan, ctx: &dyn BuiltinCtx) -> Result<LxVal, LxError> {
...
fn bi_from(args: &[LxVal], span: SourceSpan, _ctx: &dyn BuiltinCtx) -> Result<LxVal, LxError> {
  let source = &args[0];
  match source {
    LxVal::List(items) => {
...
fn bi_collect(args: &[LxVal], span: SourceSpan, ctx: &dyn BuiltinCtx) -> Result<LxVal, LxError> {
...
fn bi_map(args: &[LxVal], span: SourceSpan, _ctx: &dyn BuiltinCtx) -> Result<LxVal, LxError> {
  let inner_id = stream_id(&args[1], span)?;
  let id = alloc_stream(StreamState::Map { inner_id, func: args[0].clone() });
  Ok(LxVal::Stream { id })
}

fn bi_filter(args: &[LxVal], span: SourceSpan, _ctx: &dyn BuiltinCtx) -> Result<LxVal, LxError> {
  let inner_id = stream_id(&args[1], span)?;
  let id = alloc_stream(StreamState::Filter { inner_id, pred: args[0].clone() });
  Ok(LxVal::Stream { id })
}

fn bi_take(args: &[LxVal], span: SourceSpan, _ctx: &dyn BuiltinCtx) -> Result<LxVal, LxError> {
...
fn bi_batch(args: &[LxVal], span: SourceSpan, _ctx: &dyn BuiltinCtx) -> Result<LxVal, LxError> {
...
fn bi_each(args: &[LxVal], span: SourceSpan, ctx: &dyn BuiltinCtx) -> Result<LxVal, LxError> {
  let f = &args[0];
...
fn bi_fold(args: &[LxVal], span: SourceSpan, ctx: &dyn BuiltinCtx) -> Result<LxVal, LxError> {
