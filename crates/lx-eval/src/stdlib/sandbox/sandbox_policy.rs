...
pub(super) fn parse_policy(config: &IndexMap<lx_span::sym::Sym, LxVal>, span: SourceSpan) -> Result<Policy, LxError> {
  let mut p = make_preset("pure");

  if let Some(LxVal::Record(fs)) = config.get(&lx_span::sym::intern("fs")) {
    if let Some(v) = fs.get(&lx_span::sym::intern("read")) {
      p.fs_read = extract_string_list(v);
    }
    if let Some(v) = fs.get(&lx_span::sym::intern("write")) {
      p.fs_write = extract_string_list(v);
    }
  }

  if let Some(LxVal::Record(net)) = config.get(&lx_span::sym::intern("net"))
    && let Some(v) = net.get(&lx_span::sym::intern("allow"))
  {
    p.net_allow = extract_string_list(v);
  }

  if let Some(LxVal::Bool(b)) = config.get(&lx_span::sym::intern("agent")) {
    p.agent = *b;
  }
  if let Some(LxVal::Bool(b)) = config.get(&lx_span::sym::intern("mcp")) {
    p.mcp = *b;
  }
  if let Some(LxVal::Bool(b)) = config.get(&lx_span::sym::intern("llm")) {
    p.llm = *b;
  }
  if let Some(LxVal::Bool(b)) = config.get(&lx_span::sym::intern("embed")) {
    p.embed = *b;
  }
  if let Some(LxVal::Bool(b)) = config.get(&lx_span::sym::intern("pane")) {
    p.pane = *b;
  }

  if let Some(v) = config.get(&lx_span::sym::intern("max_time_ms")) {
    match v {
