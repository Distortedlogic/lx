...
fn render_node(val: &LxVal) -> Option<String> {
  ...
  Some(match t.as_str() {
    "heading" => {
      let level: usize = r.get(&lx_span::sym::intern("level")).and_then(|v| v.as_int()).and_then(|n| n.try_into().ok()).unwrap_or(1);
...
fn render_items(r: &IndexMap<Sym, LxVal>, prefix: &str) -> String {
  let items = r.get(&lx_span::sym::intern("items")).and_then(|v| v.as_list()).cloned().unwrap_or_default();
...
fn render_table(r: &IndexMap<Sym, LxVal>) -> String {
  let headers = r.get(&lx_span::sym::intern("headers")).and_then(|v| v.as_list()).cloned().unwrap_or_default();
  let rows = r.get(&lx_span::sym::intern("rows")).and_then(|v| v.as_list()).cloned().unwrap_or_default();
