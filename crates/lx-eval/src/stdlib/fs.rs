...
fn bi_read(args: &[LxVal], span: SourceSpan, _ctx: &dyn BuiltinCtx) -> Result<LxVal, LxError> {
  let path = args[0].require_str("fs.read", span)?;
  Ok(wrap_io(fs::read_to_string(path).map(LxVal::str)))
}

fn bi_write(args: &[LxVal], span: SourceSpan, _ctx: &dyn BuiltinCtx) -> Result<LxVal, LxError> {
  let path = args[0].require_str("fs.write", span)?;
  let content = args[1].to_string();
  Ok(wrap_io(fs::write(path, content).map(|()| LxVal::Unit)))
}

fn bi_append(args: &[LxVal], span: SourceSpan, _ctx: &dyn BuiltinCtx) -> Result<LxVal, LxError> {
  use std::io::Write;
...
fn bi_exists(args: &[LxVal], span: SourceSpan, _ctx: &dyn BuiltinCtx) -> Result<LxVal, LxError> {
...
fn bi_rm(args: &[LxVal], span: SourceSpan, _ctx: &dyn BuiltinCtx) -> Result<LxVal, LxError> {
...
fn bi_rmdir(args: &[LxVal], span: SourceSpan, _ctx: &dyn BuiltinCtx) -> Result<LxVal, LxError> {
...
fn bi_mkdir(args: &[LxVal], span: SourceSpan, _ctx: &dyn BuiltinCtx) -> Result<LxVal, LxError> {
  let path = args[0].require_str("fs.mkdir", span)?;
  Ok(wrap_io(fs::create_dir_all(path).map(|()| LxVal::Unit)))
}

fn bi_ls(args: &[LxVal], span: SourceSpan, _ctx: &dyn BuiltinCtx) -> Result<LxVal, LxError> {
...
fn bi_stat(args: &[LxVal], span: SourceSpan, _ctx: &dyn BuiltinCtx) -> Result<LxVal, LxError> {
