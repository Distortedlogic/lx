...
fn record_to_datetime(val: &LxVal, span: SourceSpan) -> Result<DateTime<Utc>, LxError> {
  match val {
    LxVal::Record(fields) => {
      if let Some(LxVal::Int(epoch)) = fields.get(&lx_span::sym::intern("epoch")) {
      ...
      if let Some(LxVal::Int(ms)) = fields.get(&lx_span::sym::intern("ms")) {
      ...
      if let Some(LxVal::Str(iso)) = fields.get(&lx_span::sym::intern("iso")) {
