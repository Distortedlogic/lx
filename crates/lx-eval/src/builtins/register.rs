...
fn log_at(args: &[LxVal], span: SourceSpan, ctx: &dyn BuiltinCtx, level: &str) -> Result<LxVal, LxError> {
  ...
  fields.insert(lx_span::sym::intern("level"), LxVal::str(level));
  fields.insert(lx_span::sym::intern("msg"), LxVal::str(s));
...
pub fn register(env: &Env) {
  ...
  log_fields.insert(lx_span::sym::intern("info"), mk("log.info", 1, bi_log_info));
  log_fields.insert(lx_span::sym::intern("warn"), mk("log.warn", 1, bi_log_warn));
  log_fields.insert(lx_span::sym::intern("err"), mk("log.err", 1, bi_log_err));
  log_fields.insert(lx_span::sym::intern("debug"), mk("log.debug", 1, bi_log_debug));
  ...
  json_fields.insert(lx_span::sym::intern("parse"), mk("json.parse", 1, bi_json_parse));
  json_fields.insert(lx_span::sym::intern("encode"), mk("json.encode", 1, bi_json_encode));
  json_fields.insert(lx_span::sym::intern("encode_pretty"), mk("json.encode_pretty", 1, bi_json_encode_pretty));
  env.bind_str("json", LxVal::record(json_fields));

  let mut agent_fields = IndexMap::new();
  agent_fields.insert(
    lx_span::sym::intern("spawn"),
    super::mk_async("agent.spawn", 1, |args, span, ctx| Box::pin(crate::builtins::agent::bi_agent_spawn(args, span, ctx))),
  );
  agent_fields.insert(
    lx_span::sym::intern("kill"),
  ...
  agent_fields.insert(lx_span::sym::intern("implements"), mk("agent.implements", 2, bi_agent_implements));
  env.bind_str("agent", LxVal::record(agent_fields));

  let mut llm_fields = IndexMap::new();
  llm_fields.insert(lx_span::sym::intern("prompt"), mk("llm.prompt", 1, super::llm::bi_prompt));
  llm_fields.insert(lx_span::sym::intern("prompt_with"), mk("llm.prompt_with", 1, super::llm::bi_prompt_with));
  llm_fields.insert(lx_span::sym::intern("prompt_structured"), mk("llm.prompt_structured", 2, super::llm::bi_prompt_structured));
  env.bind_str("llm", LxVal::record(llm_fields));

  let mut ctx_fields = IndexMap::new();
  ctx_fields.insert(lx_span::sym::intern("current"), mk("context.current", 1, bi_global_context_current));
  ctx_fields.insert(lx_span::sym::intern("get"), mk("context.get", 1, bi_global_context_get));
