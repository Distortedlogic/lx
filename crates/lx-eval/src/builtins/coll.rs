...
fn bi_first(args: &[LxVal], sp: SourceSpan, _ctx: &dyn BuiltinCtx) -> Result<LxVal, LxError> {
  Ok(args[0].require_list("first", sp)?.first().cloned().unwrap_or(LxVal::None))
}

fn bi_last(args: &[LxVal], sp: SourceSpan, _ctx: &dyn BuiltinCtx) -> Result<LxVal, LxError> {
  Ok(args[0].require_list("last", sp)?.last().cloned().unwrap_or(LxVal::None))
}

fn bi_contains(args: &[LxVal], span: SourceSpan, _ctx: &dyn BuiltinCtx) -> Result<LxVal, LxError> {
  match &args[1] {
    LxVal::Str(s) => {
...
fn bi_get(args: &[LxVal], span: SourceSpan, _ctx: &dyn BuiltinCtx) -> Result<LxVal, LxError> {
  match &args[1] {
    ...
    LxVal::Record(r) => {
      ...
      Ok(r.get(&lx_span::sym::intern(key)).cloned().unwrap_or(LxVal::None))
    },
...
fn bi_to_list(args: &[LxVal], span: SourceSpan, _ctx: &dyn BuiltinCtx) -> Result<LxVal, LxError> {
  match &args[0] {
...
fn bi_to_map(args: &[LxVal], span: SourceSpan, _ctx: &dyn BuiltinCtx) -> Result<LxVal, LxError> {
  match &args[0] {
...
fn bi_to_record(args: &[LxVal], span: SourceSpan, _ctx: &dyn BuiltinCtx) -> Result<LxVal, LxError> {
  let m = match &args[0] {
    LxVal::Map(m) => m,
    other => {
  ...
  for (k, v) in m.iter() {
    ...
    r.insert(lx_span::sym::intern(key), v.clone());
  }
  Ok(LxVal::record(r))
}

fn bi_keys(args: &[LxVal], span: SourceSpan, _ctx: &dyn BuiltinCtx) -> Result<LxVal, LxError> {
  match &args[0] {
...
fn bi_values(args: &[LxVal], span: SourceSpan, _ctx: &dyn BuiltinCtx) -> Result<LxVal, LxError> {
  match &args[0] {
...
fn bi_entries(args: &[LxVal], span: SourceSpan, _ctx: &dyn BuiltinCtx) -> Result<LxVal, LxError> {
  match &args[0] {
    LxVal::Map(m) => Ok(LxVal::list(m.iter().map(|(k, v)| kv_tuple(k.0.clone(), v.clone())).collect())),
