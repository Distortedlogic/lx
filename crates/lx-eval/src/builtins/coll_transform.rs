...
fn bi_sort(args: &[LxVal], span: SourceSpan, _ctx: &dyn BuiltinCtx) -> Result<LxVal, LxError> {
...
fn bi_sorted_q(args: &[LxVal], span: SourceSpan, _ctx: &dyn BuiltinCtx) -> Result<LxVal, LxError> {
  let l = args[0].require_list("sorted?", span)?;
  Ok(LxVal::Bool(l.windows(2).all(|w| cmp_values(&w[0], &w[1]).is_le())))
}

fn bi_rev(args: &[LxVal], span: SourceSpan, _ctx: &dyn BuiltinCtx) -> Result<LxVal, LxError> {
...
fn bi_flatten(args: &[LxVal], span: SourceSpan, _ctx: &dyn BuiltinCtx) -> Result<LxVal, LxError> {
...
fn bi_has_key(args: &[LxVal], span: SourceSpan, _ctx: &dyn BuiltinCtx) -> Result<LxVal, LxError> {
  match &args[1] {
    LxVal::Map(m) => Ok(LxVal::Bool(m.contains_key(&ValueKey(args[0].clone())))),
    LxVal::Record(r) => {
      let key = args[0].require_str("has_key?", span)?;
      Ok(LxVal::Bool(r.contains_key(&lx_span::sym::intern(key))))
    },
    other => Err(LxError::type_err(format!("has_key? expects Map/Record, got {}", other.type_name()), span, None)),
  }
}

fn bi_remove(args: &[LxVal], span: SourceSpan, _ctx: &dyn BuiltinCtx) -> Result<LxVal, LxError> {
  let m = match &args[1] {
    LxVal::Map(m) => m,
    other => {
