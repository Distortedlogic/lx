...
pub fn format_error(error: &str, span_info: Option<&SpanInfo>) -> String {
    match span_info {
        Some(si) => format!(
            "{RED}{BOLD}error:{RESET} {RED}{error}{RESET} {DIM}({}:{}){RESET}",
            si.start_line, si.start_col
        ),
...
pub fn format_event(event: &RuntimeEvent) -> String {
    match event {
        ...
        RuntimeEvent::AiCallComplete {
            ...
            duration_ms,
            ..
        } => format_ai_complete(response, model, *cost_usd, *duration_ms),
        ...
        RuntimeEvent::ShellResult {
            ...
            stderr,
            ..
        } => format_shell_result(*exit_code, stdout, stderr),
        RuntimeEvent::Error {
            error, span_info, ..
        } => format_error(error, span_info.as_ref()),
        ...
        RuntimeEvent::ProgramFinished {
            ...
            duration_ms,
            ..
        } => format_program_finished(result, *duration_ms),
        ...
        RuntimeEvent::MessageResponse {
            ...
            "{CYAN}[REPLY] {from_agent} -> {to_agent}:{RESET} {response} {DIM}({duration_ms}ms){RESET}"
        ),
