...
pub fn GenericPieChart(
  ...
  let chart = Chart::new()
    .tooltip(Tooltip::new().trigger(Trigger::Item).formatter(Formatter::String(fmt.to_owned())))
    .series(Pie::new().radius(vec![r.0, r.1]).data(items).avoid_label_overlap(true).label(Label::new().show(true).formatter(fmt)));
  rsx! {
    CharmingChart {
      chart,
      title,
