...
pub fn CharmingChart(
  ...
  let chart = if let Some(t) = title { chart.title(Title::new().text(t)) } else { chart };
  ...
  use_effect(move || {
    ...
    document::eval(&format!("DxCharts.initChart('{id}', {chart_json})"));
  });

  use_drop(move || {
    let id = chart_id();
    document::eval(&format!("DxCharts.disposeChart('{id}')"));
  });
