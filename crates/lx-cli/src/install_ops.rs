...
pub fn install_git(dest: &Path, url: &str, branch: Option<&str>, tag: Option<&str>, rev: Option<&str>) -> Result<String, String> {
  ...
  if let Some(r) = rev {
    let checkout = Command::new("git").arg("checkout").arg(r).current_dir(dest).output().map_err(|e| format!("git checkout failed: {e}"))?;
...
fn resolve_git_commit(repo: &Path) -> Result<String, String> {
  let output = Command::new("git").arg("rev-parse").arg("HEAD").current_dir(repo).output().map_err(|e| format!("git rev-parse failed: {e}"))?;
  if !output.status.success() {
...
pub fn add_dep_to_manifest(root: &Path, manifest: &mut RootManifest, pkg_arg: &str) -> Result<(), String> {
...
fn parse_pkg_arg(arg: &str) -> Result<(String, DepSpec), String> {
  if arg.starts_with("http://") || arg.starts_with("https://") || arg.starts_with("git@") {
    ...
    return Ok((name, DepSpec::Git { git: arg.to_string(), branch: Some("main".to_string()), tag: None, rev: None }));
  }
  let path = PathBuf::from(arg);
  if path.exists() && path.is_dir() {
    let name = path.file_name().and_then(|n| n.to_str()).unwrap_or("dep").to_string();
    return Ok((name, DepSpec::Path { path: arg.to_string() }));
  }
  Err(format!("cannot determine dependency type for '{arg}' — provide a git URL or local path"))
}

fn url_to_name(url: &str) -> String {
  url.rsplit('/').next().unwrap_or("dep").trim_end_matches(".git").to_string()
}
...
fn write_dep_to_toml(root: &Path, name: &str, spec: &DepSpec) -> Result<(), String> {
  ...
  let new_content = if content.contains("[dependencies]") {
    ...
    let insert_pos = if let Some(next_section) = after[1..].find('[') { idx + 1 + next_section } else { content.len() };
