...
- `Stmt::EffectDecl { name: String, params: Vec<Param>, ret: Option<SType> }` — effect type declaration
...
- `crates/lx/src/lexer/keywords.rs` — add `effect`, `perform`, `handle`, `resume` keywords
- `crates/lx/src/parser/prefix.rs` — parse `perform` and `handle` expressions
- `crates/lx/src/parser/statements.rs` — parse `effect` declarations
- `crates/lx/src/interpreter/eval.rs` — evaluate Perform and Handle
- `crates/lx/src/interpreter/mod.rs` — add effect handler table to interpreter state
- `crates/lx/src/env.rs` — store effect handlers in environment
- `crates/lx/src/checker/synth.rs` — type-check effects
- `crates/lx/src/visitor/walk/mod.rs` — walk new AST nodes

**New files:**
- `crates/lx/src/interpreter/effects.rs` — effect handler resolution and resume machinery
- `tests/81_effects.lx` — tests for algebraic effects
...
**Description:** In `crates/lx/src/lexer/keywords.rs`, add `effect`, `perform`, `handle`, `resume` as keywords.

In `crates/lx/src/ast/mod.rs`, add to the `Stmt` enum: `EffectDecl { name: String, params: Vec<Param>, ret_type: Option<SType>, span: Span }`.
...
**Description:** In `crates/lx/src/parser/statements.rs`, parse `effect Name = (params) -> RetType` as `Stmt::EffectDecl`.

In `crates/lx/src/parser/prefix.rs`, parse `perform EffectName args...` as `Expr::Perform` — consume the effect name as an identifier, then parse arguments.

Parse `handle { body } with { EffectName params -> handler_body; ... }` as `Expr::Handle`. The `with` block uses the same arm parsing as `match` but the pattern position is `EffectName param_bindings`.
...
In `crates/lx/src/env.rs`, add `effect_handlers: Option<Vec<EffectHandlerEntry>>` to the `Env` struct. The `handle` expression installs handlers in a child env. `perform` walks the env chain (via parent pointers) to find the nearest handler for the named effect.
...
For the initial implementation, use a simple approach: `perform` returns a special `EffectSignal` error (like `BreakSignal`) that carries the effect name, args, and a resume continuation. The `handle` block catches this signal, matches the effect name, evaluates the handler body, and if `resume` is called, re-invokes the body with the perform site replaced.
...
Create `tests/81_effects.lx` with tests:
...
**Description:** In `crates/lx/src/checker/stmts.rs`, handle `Stmt::EffectDecl`: register the effect name with its parameter types and return type in the checker's scope.

In `crates/lx/src/checker/synth.rs`:
