...
pub fn TerminalView(terminal_id: String, working_dir: String, command: Option<String>) -> Element {
  ...
  use_future(move || {
    ...
    async move {
      let session = match lx_ui::pty_session::get_or_create(&element_id, 80, 24, Some(&wd), cmd.as_deref()) {
        Ok(s) => s,
        Err(e) => {
  ...
  use_future(move || {
    ...
    async move {
      ...
      while let Ok(msg) = widget.recv::<serde_json::Value>().await {
        ...
        {
          let line = src[..offset as usize].chars().filter(|&c| c == '\n').count() + 1;
      ...
      match rx.recv().await {
        Ok(event) => {
          let status_update = match &event {
            lx_dx::event::RuntimeEvent::AgentSpawned { name, .. } => Some((name.clone(), "running")),
            lx_dx::event::RuntimeEvent::AiCallStart { agent_id, .. } => Some((agent_id.clone(), "active")),
            lx_dx::event::RuntimeEvent::AiCallComplete { agent_id, .. } => Some((agent_id.clone(), "completed")),
            lx_dx::event::RuntimeEvent::AiCallError { agent_id, .. } => Some((agent_id.clone(), "error")),
            lx_dx::event::RuntimeEvent::Error { agent_id, .. } => Some((agent_id.clone(), "error")),
            lx_dx::event::RuntimeEvent::AgentKilled { agent_id, .. } => Some((agent_id.clone(), "completed")),
            _ => None,
          };
