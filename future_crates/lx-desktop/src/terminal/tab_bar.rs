...
use lx_ui::tab_state::{NotificationLevel, TabsState};
use uuid::Uuid;

use super::add_tab;

#[component]
pub fn TabBar(tabs_state: Signal<TabsState>, on_new_tab: EventHandler<()>) -> Element {
      ...
      div { class: "flex items-center bg-gray-950",
              ...
              for tab in tabs.iter() {
                  {
                      ...
                      let dot_class = highest_level.map(|level| match level {
                          NotificationLevel::Error => "w-1.5 h-1.5 rounded-full inline-block ml-1.5 bg-red-500",
                          NotificationLevel::Attention => "w-1.5 h-1.5 rounded-full inline-block ml-1.5 bg-amber-500 animate-pulse",
                          NotificationLevel::Warning => "w-1.5 h-1.5 rounded-full inline-block ml-1.5 bg-amber-400",
                          NotificationLevel::Success => "w-1.5 h-1.5 rounded-full inline-block ml-1.5 bg-emerald-500",
                          NotificationLevel::Info => "w-1.5 h-1.5 rounded-full inline-block ml-1.5 bg-blue-400",
                      });
                      rsx! {
                          button {
                              ...
                              onclick: move |_| {
                                  ...
                                  s.set_active_and_focus(tab_id_click.clone());
          ...
          div { class: "relative",
              ...
              if dropdown_open() {
                  ...
                  div {
                      class: "absolute top-full right-0 z-30 mt-1 py-1 bg-gray-800 border border-gray-600 rounded-md shadow-lg min-w-52",
                      button {
                      ...
                      button {
                          class: "w-full text-left px-3 py-1.5 text-sm hover:bg-gray-700",
                          onclick: move |_| dropdown_input.set(Some((PaneKind::FlowGraph, String::new()))),
                          "\u{25C8} Flow Graph"
                      }
...
fn render_dropdown_input(tabs_state: Signal<TabsState>, dropdown_input: &Signal<Option<(PaneKind, String)>>, dropdown_open: &Signal<bool>) -> Element {
                    ...
                    rsx! {
                        button {
                            ...
                            onclick: move |_| {
                                ...
                                let node = PaneNode::Canvas {
                                    id: id.clone(),
                                    widget_type: wt.clone(),
                                    config: serde_json::Value::Object(Default::default()),
                                };
                                add_tab(tabs_state, id, wt.clone(), node);
                                dropdown_open.set(false);
                                dropdown_input.set(None);
                            },
                            "{wt2}"
  ...
  let placeholder = match kind {
    ...
    PaneKind::FlowGraph => "Enter .lx file path...",
    _ => "",
  };
...
fn create_pane_from_input(tabs_state: Signal<TabsState>, kind: PaneKind, input: &str) {
  let id = Uuid::new_v4().to_string();
  let (title, node) = match kind {
    PaneKind::Browser => {
      let url = input.to_string();
      let title = url.split('/').nth(2).unwrap_or(&url).to_string();
      let node = PaneNode::Browser { id: id.clone(), url, devtools: false };
      (title, node)
    },
    PaneKind::Editor => {
      let file_path = input.to_string();
      let title = file_path.rsplit('/').next().unwrap_or(&file_path).to_string();
      let node = PaneNode::Editor { id: id.clone(), file_path, language: None };
      (title, node)
    },
    PaneKind::Agent => {
      ...
      let node = PaneNode::Agent { id: id.clone(), session_id, model };
      (title, node)
    },
    PaneKind::FlowGraph => {
      let source_path = input.to_string();
      let title = source_path.rsplit('/').next().unwrap_or(&source_path).to_string();
      let node = PaneNode::FlowGraph { id: id.clone(), source_path };
      (title, node)
    },
    _ => return,
  };
  add_tab(tabs_state, id, title, node);
}

fn highest_notification_level(state: &TabsState, tab_id: &str) -> Option<NotificationLevel> {
