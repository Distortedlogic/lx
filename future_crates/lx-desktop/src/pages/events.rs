...
pub fn Events() -> Element {
  ...
  let visible: Vec<_> = events.read().iter().filter(|e| matches_filter(e, &current_filter)).cloned().collect();
...
fn matches_filter(event: &RuntimeEvent, filter: &str) -> bool {
  match filter {
    "all" => true,
    "ai" => matches!(event, RuntimeEvent::AiCallStart { .. } | RuntimeEvent::AiCallComplete { .. } | RuntimeEvent::AiCallError { .. }),
    "emit" => matches!(event, RuntimeEvent::Emit { .. }),
    "log" => matches!(event, RuntimeEvent::Log { .. }),
    "shell" => matches!(event, RuntimeEvent::ShellExec { .. } | RuntimeEvent::ShellResult { .. }),
    "messages" => matches!(
      event,
    ...
    "agents" => matches!(event, RuntimeEvent::AgentSpawned { .. } | RuntimeEvent::AgentKilled { .. }),
    "progress" => matches!(
      event,
