...
use crate::terminal::view::{AgentView, BrowserView, CanvasView, ChartView, EditorView, FlowGraphView, TerminalView};

fn create_new_tab(tabs_state: Signal<TabsState>) {
  let working_dir =
    tabs_state.read().focused_working_dir().or_else(|| std::env::current_dir().ok().map(|p| p.display().to_string())).unwrap_or_else(|| ".".into());
    ...
    match state.active_tab() {
      Some(tab) => {
        ...
        match source.map(|(p, _)| p) {
          Some(PaneNode::Terminal { working_dir, .. }) => PaneNode::Terminal { id: new_id, working_dir: working_dir.clone(), command: None },
          _ => PaneNode::Terminal { id: new_id, working_dir: ".".into(), command: None },
        }
      },
      None => PaneNode::Terminal { id: new_id, working_dir: ".".into(), command: None },
    }
  };
...
fn render_tab(tabs_state: Signal<TabsState>, tab: &lx_ui::tab_state::TerminalTab, active_tab_id: &Option<String>, focused_pane_id: &Option<String>) -> Element {
...
fn render_pane_item(mut tabs_state: Signal<TabsState>, pane: &PaneNode, rect: &Rect, focused_pane_id: &Option<String>) -> Element {
...
fn render_pane_view(pane: &PaneNode) -> Element {
  match pane {
    PaneNode::Terminal { id, working_dir, command } => rsx! {
        TerminalView {
    ...
    PaneNode::Editor { id, file_path, language } => rsx! {
        EditorView {
            editor_id: id.clone(),
    ...
    PaneNode::Agent { id, session_id, model } => rsx! {
        AgentView {
            agent_id: id.clone(),
    ...
    PaneNode::Canvas { id, widget_type, config } => rsx! {
        CanvasView {
            canvas_id: id.clone(),
    ...
    PaneNode::Chart { id, chart_json, title } => rsx! {
        ChartView {
            chart_id: id.clone(),
...
fn render_divider_item(mut tabs_state: Signal<TabsState>, divider: &lx_ui::pane_tree::DividerInfo, container_id: &str) -> Element {
