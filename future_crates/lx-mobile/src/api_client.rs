...
impl LxClient {
  ...
  pub async fn fetch_run_status(&self) -> Result<RunStatusResponse, String> {
    ...
    self.http.get(&url).send().await.map_err(|e| e.to_string())?.json::<RunStatusResponse>().await.map_err(|e| e.to_string())
  }
  ...
  pub async fn fetch_pending_prompts(&self) -> Result<Vec<serde_json::Value>, String> {
    ...
    self.http.get(&url).send().await.map_err(|e| e.to_string())?.json::<Vec<serde_json::Value>>().await.map_err(|e| e.to_string())
  }

  pub async fn post_user_response(&self, prompt_id: u64, response: serde_json::Value) -> Result<(), String> {
    let url = format!("{}/api/run/respond", self.base_url);
    let body = serde_json::json!({
        "prompt_id": prompt_id,
        "response": response,
    });
    self.http.post(&url).json(&body).send().await.map_err(|e| e.to_string())?;
    Ok(())
  }
}
