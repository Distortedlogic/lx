...
impl EventCategory {
  pub fn all() -> HashSet<Self> {
    HashSet::from([Self::Ai, Self::Emit, Self::Log, Self::Shell, Self::Messages, Self::Agents, Self::Progress, Self::Errors])
  }
}
...
impl App {
  ...
  pub fn categorize(event: &RuntimeEvent) -> EventCategory {
    match event {
      RuntimeEvent::AiCallStart { .. } | RuntimeEvent::AiCallComplete { .. } | RuntimeEvent::AiCallError { .. } => EventCategory::Ai,
      RuntimeEvent::Emit { .. } => EventCategory::Emit,
      RuntimeEvent::Log { .. } => EventCategory::Log,
      RuntimeEvent::ShellExec { .. } | RuntimeEvent::ShellResult { .. } => EventCategory::Shell,
      ...
      RuntimeEvent::AgentSpawned { .. } | RuntimeEvent::AgentKilled { .. } => EventCategory::Agents,
  ...
  pub fn push_event(&mut self, event: RuntimeEvent) {
    match &event {
      RuntimeEvent::AiCallComplete { cost_usd: Some(cost), .. } => {
      ...
      RuntimeEvent::ProgramFinished { result, duration_ms, .. } => {
