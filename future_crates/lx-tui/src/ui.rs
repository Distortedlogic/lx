...
pub fn render(app: &App, frame: &mut Frame) {
  let chunks = Layout::vertical([Constraint::Length(3), Constraint::Min(1), Constraint::Length(2)]).split(frame.area());
...
fn render_events(app: &App, frame: &mut Frame, area: ratatui::layout::Rect) {
  ...
  let scroll_offset = if app.scroll > items.len().saturating_sub(1) { items.len().saturating_sub(1) } else { app.scroll };

  let block = Block::default().borders(Borders::ALL).title(format!("Events ({visible_count}/{total})"));
  ...
  if visible_count > inner_height && scroll_offset + inner_height < visible_count {
    ...
    let indicator_area = ratatui::layout::Rect { x: area.x + area.width - 2, y: area.y + area.height - 2, width: 1, height: 1 };
    frame.render_widget(indicator, indicator_area);
  }
}

fn render_footer(app: &App, frame: &mut Frame, area: ratatui::layout::Rect) {
  let hotkeys = Line::from(vec![Span::raw("a:AI  e:Emit  l:Log  s:Shell  m:Msg  g:Agent  p:Prog  r:Err  *:All  Tab:Agent  q:Quit")]);

  let mut filter_spans = Vec::new();
  for (cat, label, color) in category_display_info() {
    let style = if app.filters.contains(&cat) { Style::default().fg(color) } else { Style::default().fg(Color::DarkGray) };
    filter_spans.push(Span::styled(label, style));
    filter_spans.push(Span::raw("  "));
  }
  if let Some(ref agent) = app.agent_filter {
    filter_spans.push(Span::styled(format!("agent: {agent}"), Style::default().fg(Color::White).add_modifier(Modifier::BOLD)));
  }
...
fn event_to_spans(event: &RuntimeEvent) -> Vec<Span<'_>> {
  ...
  let color = match event {
    ...
    RuntimeEvent::Progress { .. } | RuntimeEvent::ProgramStarted { .. } | RuntimeEvent::ProgramFinished { .. } | RuntimeEvent::TraceSpanRecorded { .. } => {
      Color::Magenta
    },
  };
