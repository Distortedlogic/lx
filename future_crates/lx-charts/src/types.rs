...
impl DataZoomConfig {
  pub fn last_n(data_len: usize, visible: usize) -> Self {
    let start = if data_len > visible { (data_len.saturating_sub(visible)) as f64 / data_len.max(1) as f64 * 100.0 } else { 0.0 };
    Self { start, end: 100.0, slider: true, inside: true }
  }
}

impl Default for DataZoomConfig {
  fn default() -> Self {
    Self { start: 0.0, end: 100.0, slider: true, inside: true }
  }
}
