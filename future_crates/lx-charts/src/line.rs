...
use charming::{
  ...
  element::{AreaStyle, AxisType, Color, Emphasis, EmphasisFocus, ItemStyle, LineStyle, Orient, Tooltip, Trigger},
  series::Line,
};
use dioxus::prelude::*;
...
pub fn GenericLineChart(
  ...
  let mut chart = Chart::new().tooltip(Tooltip::new().trigger(Trigger::Axis)).x_axis(x_axis).y_axis(y_axis);
  ...
  for s in &series {
    ...
    line = line
      ...
      .line_style(LineStyle::new().color(Color::from(s.color)).width(s.width.unwrap_or(2)))
  ...
  if legend {
    ...
    let mut leg = Legend::new().data(legend_data).selected_mode(LegendSelectedMode::Multiple).show(legend_show.unwrap_or(true));
    match pos {
  ...
  if is_expanded {
    ...
    if zoom.slider {
      chart = chart.data_zoom(DataZoom::new().type_(DataZoomType::Slider).start(zoom.start).end(zoom.end));
    }
    if zoom.inside {
      chart = chart.data_zoom(DataZoom::new().type_(DataZoomType::Inside).start(zoom.start).end(zoom.end));
    }
  }
  rsx! {
    CharmingChart {
