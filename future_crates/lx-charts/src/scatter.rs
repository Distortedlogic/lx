...
pub fn GenericScatterChart(
  ...
  let mut chart = Chart::new().tooltip(Tooltip::new().trigger(Trigger::Item)).x_axis(x_axis).y_axis(y_axis);
  for s in &series {
    let mut scatter = Scatter::new().name(&s.name).data(s.data.clone()).item_style(ItemStyle::new().color(Color::from(s.color)));
