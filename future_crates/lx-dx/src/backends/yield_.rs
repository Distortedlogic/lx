...
impl YieldBackend for DxYieldBackend {
  fn yield_value(&self, value: LxVal, span: Span) -> Result<LxVal, LxError> {
    ...
    self.bus.send(RuntimeEvent::Emit { agent_id: self.agent_id.clone(), value: format!("[yield] {json_val}"), ts: Instant::now() });

    let (tx, rx) = oneshot::channel();
    {
      let mut guard = self.response_tx.lock().map_err(|e| LxError::runtime(format!("yield lock: {e}"), span))?;
      *guard = Some(tx);
    }
    ...
    self.bus.send(RuntimeEvent::UserPrompt {
      ...
      prompt_id,
      kind: crate::event::UserPromptKind::Ask { message: format!("yield: {json_val}"), default: None },
      ts: Instant::now(),
    });

    let response = rx.blocking_recv().map_err(|_| LxError::runtime("yield: orchestrator cancelled", span))?;

    self.bus.send(RuntimeEvent::UserResponse { agent_id: self.agent_id.clone(), prompt_id, response: response.clone(), ts: Instant::now() });
