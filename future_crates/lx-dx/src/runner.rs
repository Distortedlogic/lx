...
impl ProgramRunner {
  ...
  pub async fn run(&self, source_path: &str) -> Result<LxVal, RunError> {
    let source = tokio::fs::read_to_string(source_path).await.map_err(|e| RunError::Io(format!("read {source_path}: {e}")))?;

    let tokens = lexer::lex(&source).map_err(RunError::Lx)?;
    let program = parser::parse(tokens).map_err(RunError::Lx)?;

    let trace = self.langfuse.create_trace(source_path, serde_json::json!({ "source_path": source_path }));

    let agent_id = "main".to_string();
    let ctx = build_runtime_ctx(self.bus.clone(), self.langfuse.clone(), agent_id);

    self.bus.send(RuntimeEvent::ProgramStarted { source_path: source_path.to_string(), ts: Instant::now() });
