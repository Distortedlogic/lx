...
impl<W: Write + Send + 'static> PtyWriter<W> {
  ...
  pub fn write_event(&self, event: &RuntimeEvent) -> io::Result<()> {
    ...
    let mut w = self.writer.lock().map_err(|e| io::Error::other(format!("lock: {e}")))?;
...
pub fn spawn_pty_writer<W: Write + Send + 'static>(bus: &EventBus, agent_id: String, writer: W) -> tokio::task::JoinHandle<()> {
    ...
    loop {
      match rx.recv().await {
        Ok(event) => {
          ...
          let matches = event.agent_id().is_some_and(|id| id == agent_id);
          if matches && let Err(e) = pty.write_event(&event) {
