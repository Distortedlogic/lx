...
impl LangfuseClient {
  pub fn from_env() -> Self {
    ...
    let base_url = std::env::var("LANGFUSE_BASE_URL").unwrap_or_else(|_| DEFAULT_BASE_URL.to_string());
    ...
    Self { base_url, auth_header, http: Client::new(), enabled }
  }
  ...
  pub fn create_trace<'a>(&'a self, name: &str, metadata: Value) -> LangfuseTrace<'a> {
    ...
    LangfuseTrace { id: trace_id, client: self }
  }

  pub fn create_generation<'a>(&'a self, trace: &LangfuseTrace<'a>, name: &str, model: &str, input: &str) -> LangfuseGeneration<'a> {
    ...
    LangfuseGeneration { id: gen_id, trace_id: trace.id.clone(), client: self }
  }

  pub fn create_span<'a>(&'a self, trace: &LangfuseTrace<'a>, name: &str, input: &str) -> LangfuseSpan<'a> {
    ...
    LangfuseSpan { id: span_id, trace_id: trace.id.clone(), client: self }
  }
  ...
  fn fire_and_forget(&self, endpoint: &str, body: Value) {
    ...
    let req = self.http.post(url).header("Authorization", auth).header("Content-Type", "application/json").json(&body);
...
impl<'a> LangfuseTrace<'a> {
  pub fn end(&self, success: bool) {
    ...
    let req = self.client.http.patch(url).header("Authorization", auth).header("Content-Type", "application/json").json(&body);
    tokio::spawn(async move {
      let _ = req.send().await;
    });
  }
...
impl<'a> LangfuseGeneration<'a> {
  ...
  fn end_with(&self, body: Value) {
    ...
    let url = format!("{}/api/public/generations/{}", self.client.base_url, self.id);
    let req = self.client.http.patch(url).header("Authorization", auth).header("Content-Type", "application/json").json(&body);
    tokio::spawn(async move {
      let _ = req.send().await;
    });
