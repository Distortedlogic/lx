...
pub fn get_or_create(id: &str, cols: u16, rows: u16, working_dir: Option<&str>, command: Option<&str>) -> Result<Arc<PtySession>, String> {
...
impl PtySession {
  fn spawn(cols: u16, rows: u16, working_dir: Option<&str>, command: Option<&str>) -> Result<Self, String> {
    ...
    Ok(Self { input_tx: in_tx, output_tx: out_tx, buffer, master: parking_lot::Mutex::new(pair.master), _child: child })
  }
  ...
  pub fn resize(&self, cols: u16, rows: u16) -> Result<(), String> {
    self.master.lock().resize(PtySize { rows, cols, pixel_width: 0, pixel_height: 0 }).map_err(|e| e.to_string())
  }
}
