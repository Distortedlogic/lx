...
impl PaneNode {
  ...
  pub fn compute_dividers(&self, rect: Rect) -> Vec<DividerInfo> {
    match self {
      ...
      | Self::Chart { .. }
      | Self::FlowGraph { .. } => vec![],
      Self::Split { id: sid, direction, ratio, first, second } => {
        let (first_rect, second_rect) = split_rect(rect, *direction, *ratio);
        let divider_rect = match direction {
          SplitDirection::Horizontal => Rect { left: first_rect.left + first_rect.width, top: rect.top, width: 0.0, height: rect.height },
          SplitDirection::Vertical => Rect { left: rect.left, top: first_rect.top + first_rect.height, width: rect.width, height: 0.0 },
        };
        let mut result = vec![DividerInfo { rect: divider_rect, parent_rect: rect, direction: *direction, split_id: sid.clone() }];
  ...
  pub fn first_terminal_id(&self) -> Option<String> {
    match self {
      ...
      Self::Browser { .. } | Self::Editor { .. } | Self::Agent { .. } | Self::Canvas { .. } | Self::Chart { .. } | Self::FlowGraph { .. } => None,
      Self::Split { first, second, .. } => first.first_terminal_id().or_else(|| second.first_terminal_id()),
    }
  }

  pub fn find_working_dir(&self, target_id: &str) -> Option<String> {
    match self {
      Self::Terminal { id, working_dir, .. } if id == target_id => Some(working_dir.clone()),
      Self::Editor { id, file_path, .. } if id == target_id => std::path::Path::new(file_path).parent().map(|p| p.to_string_lossy().into_owned()),
      Self::Browser { id, .. } | Self::Agent { id, .. } | Self::Canvas { id, .. } | Self::Chart { id, .. } | Self::FlowGraph { id, .. } if id == target_id => {
        None
      },
      Self::Split { first, second, .. } => first.find_working_dir(target_id).or_else(|| second.find_working_dir(target_id)),
      Self::Terminal { .. }
      | Self::Browser { .. }
      | Self::Editor { .. }
      | Self::Agent { .. }
  ...
  match direction {
    SplitDirection::Horizontal => {
      ...
      (
        ...
        Rect { left: rect.left + first_width + DIVIDER_SIZE_PCT, top: rect.top, width: second_width, height: rect.height },
      )
    },
    SplitDirection::Vertical => {
      ...
      (
        ...
        Rect { left: rect.left, top: rect.top + first_height + DIVIDER_SIZE_PCT, width: rect.width, height: second_height },
      )
    },
  }
}
